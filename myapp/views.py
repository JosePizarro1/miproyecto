...
def tesoreria_home(request):
    ...
    return render(request, 'tesoreria_home.html', {
        ...
        'rol': rol_usuario  # Incluir el rol del usuario en el contexto
...
def subir_documento_view(request):
    if request.method == 'POST':
        ...
        asunto = request.POST['asunto']  # Obtener el asunto del formulario
        archivo = request.FILES['archivo']
        
        # Renombrar el archivo para eliminar caracteres especiales (tildes, etc.)
        nombre_archivo, extension_archivo = os.path.splitext(archivo.name)
        nombre_archivo_normalizado = unidecode(nombre_archivo)  # Remueve tildes y caracteres especiales
        nombre_archivo_slug = slugify(nombre_archivo_normalizado)  # Alternativa: genera un slug limpio
        nuevo_nombre_archivo = f"{nombre_archivo_slug}{extension_archivo}"

        # Reemplazar el nombre original con el nombre normalizado
        ...
        # Crear el documento
        documento = Documento(usuario=request.user, tipo=tipo, asunto=asunto, archivo=archivo)  # Añadir el asunto aquí
